...
intrinsic ExtendMapToAlgebra(input::SeqEnum[ParAxlAlgElt],images::SeqEnum[ParAxlAlgElt])->BoolElt,AlgMatElt
{
...
/*We have an optional input for the identity if it exists, naturally inputting it will speed up things.*/
{
	...
	if form eq IdentityMatrix(F,n) then
		...
		if bool1 eq true then
			...
			eigs:={x[1]:x in Eigenvalues(aut)};/*Shorten this. TOO LONG.*/
...
/*here we can as well just input the tensor. TO DO*/
...
intrinsic HasIdentitySubAlg(A::ParAxlAlg, V::ModTupFld)-> BoolElt, ParAxlAlgElt
{
	...
	tens:=[];
	d:=Dimension(V);
	W:=VectorSpace(Rationals(),Dimension(A)); /*now can shorten this bu using get structure constants.TO DO*/
	basmat:=Matrix(Rationals(),[Eltseq(V.i):i in [1..d]]);
	for i:=1 to d do 
		for j:=1 to d do 
			if i le j then
...
intrinsic IdentifyShape(lst::SeqEnum)->MonStgElt 
{
	...
	if ord eq 2 and dim eq 3 then /* the 2A case.*/
		...
		tens:=SubAlgStructure(IndexedSet(bas));/*Change to GetStructureConstants, remove SubAlgStructure.*/
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
+ Suppose that you have a list, say lst, of axes. What shape is the      +
+ algebra generated by these? This will require your list to be axial    + 
+ vectors. This calls IdentifyShape, so the file must be loaded into the +
+ session.                                                               +
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
intrinsic IdentifyShapeSubAlg(L::SeqEnum[ParAxlAlgElt])->StgElt
{
	...
	shapes:=[];
 	shape:="";
  	for i in [1..#L] do
		for j:=i+1 to #L do /* axial algebras are commutative.*/ 
			shape1:=IdentifyShape(L[[i,j]]); 
	 		if not shape1 in shapes then
		 		shape*:=shape1;
		 		Append(~shapes,shape1);
	 		end if;
		end for;
	end for;
	return shape;
